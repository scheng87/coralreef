...
```{r gg, fig.cap="**Figure 1.** Growth in coral reef publications over the last 15 years, disaggregated by journal", echo=FALSE}
...
**Fix figure titles. First graph - proportion of women vs. men as one bar**
</span>
...
```{r gender_time, fig.cap='**Figure X.** Change in composition of gender in first, last, and single author positions over time', echo=FALSE}
##Over time
...
```{r gender_prop_time, fig.cap='**Figure X.** Change in proportion of individual contribution of genders over time', echo=FALSE}
...
fig_affil <- ggarrange(                                              
          ggarrange(a,b, ncol = 2, labels = c("A", "B")),c,d,
          nrow = 3, 
          labels = c("","C","D"),
          common.legend=TRUE,
          legend="bottom"
...
```{R div_stats, echo=FALSE, fig.cap='**Figure X.** Distribution of different proportions of whether the author team based in the study country in examined articles.'}
...
```{r div_sum, echo=FALSE, fig.cap='**Figure X.** Frequency of first, last, and single authors being based in the study country'}
...
When you look at where authors are based (i.e. the country/region of affiliation) and where they do their research - we see that relatively fewer study locations are being studied by scientists based in that country, with the exception of the US and Australia. North American, European, and Australian scientists by in large, are working across the globe, however, scientists from other nations are less represented.

![By Income Group](WherePeopleDoResearch_byRegion.png)

The difference is even more stark when you examine this connection map coded by the income bracket of affiliation countries. Global coral reef research is dominated by research effort originating from primarily, high income and to a lesser degree, upper middle income countries. Research is occuring in primarily low and lower-middle income countries.

![By Region](WherePeopleDoResearch_byIncomeGroup.png)

```{r div_gender_collaboration, echo=FALSE, fig.cap='**Figure X.** A). How often papers are written by a national vs. international collaboration. B).how often male vs. female first and last authored papers were part of a national or international collaboration.'}
